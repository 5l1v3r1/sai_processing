...
fname = "/Users/kolsha/Downloads/coins.jpg"
...
'''
circles = cv.HoughCircles(img, cv.HOUGH_GRADIENT, 2, 200, param1=30, param2=15, minRadius=0, maxRadius=0)
...
number_of_circles_expected = 5  # we expect to find just one circle
breakout = False
...
while guess_accumulator_array_threshold > 1 and not breakout:
...
for cir in circleLog:
    ...
    cir = np.round(cir[0, :]).astype(np.int)
